...
{
    $consulta = $conexion->prepare("DELETE FROM administradores WHERE nombre LIKE :nombre");
    $consulta->bindParam(':nombre', $nombre);
    $consulta->execute();
    return $consulta;
}
...
{
    $consulta = $conexion->prepare("SELECT donantes.nombre, donantes.apellidos, historico.fechaDonacion, historico.proximaDonacion FROM donantes, historico WHERE donantes.id=historico.idDonante AND historico.fechaDonacion BETWEEN '$desde' AND '$hasta' ORDER BY historico.fechaDonacion DESC");
    $consulta->execute();
    return $consulta;
}
